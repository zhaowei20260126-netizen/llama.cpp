...
// shared memory for TP collective ops
struct ggml_tp_shm {
    void * base;
    size_t total_size;
...
static struct ggml_tp_shm g_tp_shm = { NULL, 0, 0, 4, -1, 1 };

extern "C" void ggml_tp_shm_init(int rank, int size, void * shm_base, size_t shm_size) {
    g_tp_shm.rank  = rank;
    g_tp_shm.size  = size;
    g_tp_shm.base  = shm_base;
    g_tp_shm.total_size = shm_size;
    g_tp_shm.barrier_count  = 0;
    g_tp_shm.barrier_target = size;
}

static void tp_barrier_wait(void) {
    int old = __sync_fetch_and_add(&g_tp_shm.barrier_count, 1);
    if (old + 1 == g_tp_shm.barrier_target) {
        __sync_lock_release(&g_tp_shm.barrier_count);
    } else {
        while (__sync_fetch_and_add(&g_tp_shm.barrier_count, 0) != 0) {
...
void ggml_compute_forward_all_reduce_sum(
    ...
    const int tp_rank = g_tp_shm.rank;
    const int tp_size = g_tp_shm.size;

    GGML_ASSERT(tp_rank >= 0 && tp_rank < tp_size);
    ...
    // phase 1: each rank writes to shared window
    float * shm = (float *) g_tp_shm.base;
    float * my_slot = shm + tp_rank * nelem;
    memcpy(my_slot, src_data, nelem * sizeof(float));

    tp_barrier_wait();

    // phase 2: sum all ranks
...
void ggml_compute_forward_all_gather(
        const struct ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    GGML_ASSERT(tp_rank >= 0 && tp_rank < tp_size);
    GGML_ASSERT(src->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_nelements(dst) == nelem * tp_size);
    ...
    // phase 2: gather all ranks' data
    for (int r = 0; r < tp_size; r++) {
        memcpy(dst_data + r * nelem, shm + r * nelem, nelem * sizeof(float));
    }
