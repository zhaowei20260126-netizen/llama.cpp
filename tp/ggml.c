...
} g_tp_ctx = { -1, 1, NULL, NULL };

void ggml_set_tp_ctx(int rank, int size) {
    g_tp_ctx.rank = rank;
    g_tp_ctx.size = size;
}

int ggml_get_tp_rank(void) {
    return g_tp_ctx.rank;
}

int ggml_get_tp_size(void) {
    return g_tp_ctx.size;
}
...
struct ggml_tensor * ggml_all_reduce_sum(
    ...
    struct ggml_tensor * result = ggml_new_tensor(
        ctx, a->type, a->n_dims, a->ne);
    ggml_set_op_params_i32(result, 0, g_tp_ctx.rank);
    ggml_set_op_params_i32(result, 1, g_tp_ctx.size);
    result->op = GGML_OP_ALL_REDUCE_SUM;
    result->src[0] = a;
    return result;
}

struct ggml_tensor * ggml_all_gather(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    bool is_node = false;
    // output has tp_size × elements along last dim
    int64_t ne_out[GGML_MAX_DIMS] = {0};
    for (int i = 0; i < GGML_MAX_DIMS; i++) ne_out[i] = a->ne[i];
    ne_out[0] *= g_tp_ctx.size;  // all-gather along inner dim
    struct ggml_tensor * result = ggml_new_tensor(
        ctx, a->type, a->n_dims, ne_out);
    ggml_set_op_params_i32(result, 0, g_tp_ctx.rank);
    ggml_set_op_params_i32(result, 1, g_tp_ctx.size);
    result->op = GGML_OP_ALL_GATHER;
    result->src[0] = a;
    return result;
}
