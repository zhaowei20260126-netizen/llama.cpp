...
// TP shared memory initialization (from ggml TP ops in ops.cpp)
...
static void print_usage(const char * prog) {
    fprintf(stderr,
            ...
            "          [--hidden-dtype bf16|f32] [--quiet] [--verbose]\n"
            ...
            "          [--prompt TEXT]\n"
            "          [--prefill-chunk N] [--hidden-dtype bf16|f32] [--quiet] [--verbose]\n"
...
static llama_token read_token_for_seq(ntb_mw_transport & transport, int32_t seq_id, std::vector<token_payload> & pending) {
...
static int run_head(const pipeline_args & args, Transport & transport) {
    ...
    for (int32_t step = 1; step < args.n_predict; ++step) {
        ...
        for (int32_t seq_id = 0; seq_id < args.parallel; ++seq_id) {
            ...
            const int32_t pos = (int32_t) prompt_tokens.size() + step - 1;
            batch.token[seq_id] = token;
            set_batch_entry(batch, seq_id, seq_id, pos, true);
            metas.push_back({ seq_id, pos, PIPELINE_FLAG_WANT_LOGITS, 0u });
        }
        ...
        uint32_t flags = args.hidden_type == hidden_dtype::bf16 ? PIPELINE_FLAG_HIDDEN_BF16 : 0u;
        const int32_t pos = (int32_t) prompt_tokens.size() + step - 1;
        transport.send_hidden_payload(pos, batch.n_tokens, n_embd, flags | PIPELINE_FLAG_WANT_LOGITS, payload.data(), payload.size());
        if (!args.quiet) {
            fprintf(stderr, "pipeline-brick head: sent decode pos=%d n_tokens=%d bytes=%zu dtype=%s\n",
                    pos, batch.n_tokens, payload.size(), hidden_dtype_name(args.hidden_type));
        }
    }
...
// ---- TP (Tensor Parallelism) process management ----

// TP transport proxy: rank 0 owns the pipeline channel,
// ranks 1..N-1 receive data through TP shared memory broadcast.
template<typename Transport>
struct tp_transport_proxy {
    Transport & real;
    int        tp_rank;
    void     * tp_shm;     // shared broadcast buffer (header + payload)
    volatile int * ready;  // 0 = empty, 1 = rank 0 has written data

    void send_hidden(int32_t seq_id, int32_t pos, int32_t n_embd, uint32_t flags, const float * payload) {
        if (tp_rank == 0) real.send_hidden(seq_id, pos, n_embd, flags, payload);
    }
    void send_hidden_payload(int32_t pos, int32_t n_tokens, int32_t n_embd, uint32_t flags, const void * payload, uint64_t payload_bytes) {
        if (tp_rank == 0) real.send_hidden_payload(pos, n_tokens, n_embd, flags, payload, payload_bytes);
    }
    void send_token(int32_t seq_id, llama_token token) {
        if (tp_rank == 0) real.send_token(seq_id, token);
    }
    void send_stop(int32_t seq_id, int32_t pos) {
        if (tp_rank == 0) real.send_stop(seq_id, pos);
    }

    recv_packet recv() {
        if (tp_rank == 0) {
            recv_packet pkt = real.recv();
            // broadcast to other ranks via TP shared memory
            brick_packet_header * hdr = (brick_packet_header *)tp_shm;
            *hdr = pkt.header;
            uint32_t * meta = (uint32_t *)((uint8_t *)tp_shm + sizeof(brick_packet_header));
            meta[0] = (uint32_t)pkt.payload.size();
            if (pkt.payload.size() > 0) {
                memcpy((uint8_t *)tp_shm + sizeof(brick_packet_header) + 16,
        ...
        // non-rank-0: spin-wait for rank 0's broadcast
        ...
        recv_packet pkt;
        memcpy(&pkt.header, tp_shm, sizeof(pkt.header));
        uint32_t * meta = (uint32_t *)((uint8_t *)tp_shm + sizeof(brick_packet_header));
        uint32_t payload_size = meta[0];
        if (payload_size > 0) {
            pkt.payload.resize(payload_size);
            memcpy(pkt.payload.data(), (uint8_t *)tp_shm + sizeof(brick_packet_header) + 16, payload_size);
        }

        // only last non-rank-0 resets; barrier in TP all-reduce ensures ordering
...
static std::string make_tp_model_path(const std::string & base_path, int tp_rank) {
    // base: "qwen3-4b-F16.gguf" → "qwen3-4b-F16.tp0.gguf"
...
static size_t tp_shm_size(const pipeline_args & args) {
    // enough for all-reduce of largest intermediate (FFN h_dim=6912) × max_tokens × fp32 × tp_size
    const size_t max_elements = (size_t) 6912 * (size_t) max_micro_batch_tokens(args);
    return max_elements * sizeof(float) * (size_t) args.numa_tp + 65536;
}

static int run_hardware_tp(const pipeline_args & args) {
    const int tp_size = args.numa_tp;
    const int tp_base_numa = (args.role == LLAMA_PIPELINE_BRICK_ROLE_HEAD) ? 0 : 4;
    const size_t shm_bytes = tp_shm_size(args);

    // layout: [broadcast_buf (1MB)] [ready_flag (4B)] [allreduce_buf (rest)]
    const size_t bc_buf_size = sizeof(brick_packet_header) + 16 + max_payload_bytes + 4096;
    const size_t shm_bytes = bc_buf_size + 64 + tp_shm_size(args);

    void * tp_shm = mmap(nullptr, shm_bytes, PROT_READ | PROT_WRITE,
                         MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (tp_shm == MAP_FAILED) {
        throw std::runtime_error("TP shm mmap failed: " + std::string(strerror(errno)));
    }
    memset(tp_shm, 0, shm_bytes);

    void * tp_bc_buf  = tp_shm;                                    // broadcast buffer
    volatile int * tp_ready = (volatile int *)((uint8_t *)tp_shm + bc_buf_size); // sync flag
    void * tp_ar_buf  = (uint8_t *)tp_shm + bc_buf_size + 64;     // all-reduce buffer

    fprintf(stderr, "pipeline-brick TP: tp_size=%d tp_shm=%zu MB base_numa=%d role=%s\n",
            tp_size, shm_bytes / (1024 * 1024),
            tp_base_numa, role_name(args.role).c_str());

    const size_t max_payload = max_hidden_payload_bytes(args);
    ...
    for (int r = 0; r < tp_size; r++) {
        ...
        if (pid == 0) {
            // ---- child process ----
            const int numa_node = tp_base_numa + r;

            std::string cpu_list = cpus_for_numa_nodes(std::to_string(numa_node));
            bind_to_cpus(cpu_list);

            pipeline_args child_args = args;
            child_args.numa_tp = 1;
            child_args.model = make_tp_model_path(args.model, r);
            child_args.numa_cpus = cpu_list;

            fprintf(stderr,
                    "pipeline-brick TP: rank=%d/%d numa=%d cpu=%s model=%s\n",
                    r, tp_size, numa_node, cpu_list.c_str(), child_args.model.c_str());

            // init TP all-reduce (uses tp_ar_buf portion of shared memory)
            const size_t ar_buf_size = shm_bytes - bc_buf_size - 64;
            ggml_tp_shm_init(r, tp_size, tp_ar_buf, ar_buf_size);
            ggml_set_tp_ctx(r, tp_size);

            // create transport and wrap with TP proxy
            if (args.transport == transport_kind::cxl) {
                cxl_transport real = cxl_transport::open_transport(args, max_payload);
                tp_transport_proxy<cxl_transport> transport = {
                    real, r, tp_bc_buf, tp_ready
                };
                int rc;
                if (args.role == LLAMA_PIPELINE_BRICK_ROLE_HEAD) {
                    rc = run_head(args, transport);
                } else {
                    rc = run_tail(args, transport);
                }
                _exit(rc);
            } else {
                ntb_mw_transport real = ntb_mw_transport::open_transport(args, max_payload);
                tp_transport_proxy<ntb_mw_transport> transport = {
                    real, r, tp_bc_buf, tp_ready
                };
                int rc;
                if (args.role == LLAMA_PIPELINE_BRICK_ROLE_HEAD) {
