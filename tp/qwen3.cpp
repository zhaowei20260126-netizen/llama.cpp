...
llama_model_qwen3::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    const bool tp_enabled = params.tp_size > 1;
    ...
    for (int il = layer_start; il < layer_end; ++il) {
        ...
        // self-attention
        {
            ...
            // TP: all-gather Q/K/V after QKV projection
            if (tp_enabled) {
                Qcur = ggml_all_gather(ctx0, Qcur);
                Kcur = ggml_all_gather(ctx0, Kcur);
                Vcur = ggml_all_gather(ctx0, Vcur);
            }
        ...
        if (tp_enabled) {
            // ---- TP-aware FFN (SwiGLU, parallel gate/up) ----
            // up and gate projections (column-parallel → all-gather)
            ...
            if (model.layers[il].ffn_up_s)   ffn_up   = ggml_mul(ctx0, ffn_up,   model.layers[il].ffn_up_s);
            if (model.layers[il].ffn_gate_s) ffn_gate = ggml_mul(ctx0, ffn_gate, model.layers[il].ffn_gate_s);

            ffn_up   = ggml_all_gather(ctx0, ffn_up);
            ffn_gate = ggml_all_gather(ctx0, ffn_gate);
            ...
            if (model.layers[il].ffn_down_s) cur = ggml_mul(ctx0, cur, model.layers[il].ffn_down_s);
